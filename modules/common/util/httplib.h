...
*/
#ifndef MODULES_COMMON_UTIL_HTTPLIB_H_
#define MODULES_COMMON_UTIL_HTTPLIB_H_
    ...
    switch (s[i]) {
      ...
      default:
        if (s[i] < 0) {
          ...
          char hex[4];
          size_t len =
              snprintf(hex, sizeof(hex) - 1, "%02X", (unsigned char)s[i]);
          assert(len == 2);
...
inline void Server::write_response(Stream& strm, bool last_connection,
  ...
  if (!res.body.empty()) {
    ...
    if (chunked_response) res.set_header("Transfer-Encoding", "chunked");
  }
...
#endif  // MODULES_COMMON_UTIL_HTTPLIB_H_
